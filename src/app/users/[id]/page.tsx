...
const UserDetailPage = () => {
  ...
  const [username, setUsername] = useState(''); // username の state を追加
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      ...
      try {
        ...
        setEmail(data.email);
        setUsername(data.username || ''); // username の state を初期化
      } catch (err: any) {
  ...
  const handleCancelEdit = () => {
    ...
    setEmail(user.email);
    setUsername(user.username || ''); // キャンセル時に元の値を戻す
  };
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/users/${id}`, {
        ...
        body: JSON.stringify({
          ...
          email: email,
          username: username, // username を送信
        }),
      });
      ...
      <div className="max-w-md w-full px-4 sm:px-6 lg:px-8 bg-white rounded-md shadow-md">
        ...
        <div className="mb-4 p-4 border rounded">
          ...
          {isEditing ? (
            ...
            <>
              <p><strong>表示名:</strong> {user?.display_name || user?.username}</p>
              <p><strong>ユーザー名:</strong> {user?.username}</p> {/* 追加 */}
